...
In order to analyze differences between districts, we can determine the most 'violent' districts within each of our highlighted clusters.
...
worst1 <- clust_crime %>% 
  ...
  select(District_Num) %>%
  head(1)
worst1$District_Num[1] # District 1803 is of interest in cluster 1
```
...
# We will run this model on district 1249 within cluster 10.
...
clust_crime <- clust_crime %>%
  mutate(TimeOfDay = ifelse(hour < 4, 0,
         ifelse(hour < 8, 1,
                ifelse(hour < 12, 2,
                       ifelse(hour < 16, 3,
                              ifelse(hour < 20, 4, 5))))))
...
dist1249 <- clust_crime %>% filter(District_Num == "1249")
vars <- dist1249 %>%
  select(Crime_Code, Premis_Code, Adult_Arrest, Juv_Arrest, Gun, Blade)
outcome <-dist1249$TimeOfDay
...
Within each hour of the day, we see a balanced accuracy of between 0.50 and 0.61. Considering that we have six different periods for time of day, our model is neither a strong nor weak predictor.
...
This will help an officer know if a crime is reported at on x day at x time in x premises with x victim, wehther they should be prepared for violence.
```{r Violent crime model}
library(splitstackshape)
clust_crime2 <- na.omit(clust_crime)

#Evaluating our worst clusters
worst_clusts <- clust_crime[clust_crime$Cluster_Num != 1,]
worst_clusts$numDayOfWeek <- factor(worst_clusts$numDayOfWeek)
worst_clusts$hour <- factor(worst_clusts$hour)
worst_clusts$month <- factor(worst_clusts$month)
worst_clusts$Premis_Code <- factor(worst_clusts$Premis_Code)
worst_clusts$District_Num <- factor(worst_clusts$District_Num)
...
In this model, we see a balanced accuracy of 0.7250. We see very similar sensitivity and specificity values, meaning that our model is not skewed toward predicting one class over another.


Our next model will predict the 'type' of crime to occur. In order to predict type, we will categorize crime using the first of its three-digit crime code.
```{r crime group model}
clust_crime2$crime_group <- as.numeric(substr(as.character(clust_crime2$Crime_Code), 1, 1))

vars <- clust_crime2 %>%
  select(Adult_Arrest, Juv_Arrest, Gun, Blade, numDayOfWeek, hour)
outcome <-clust_crime2$crime_group - 1
...
type_model <- xgb.train(data = dtrain, 
                   nrounds = 100,
                   eta = 0.1,
                   objective = "multi:softmax",
                   num_class = 9,
...
predictions <- predict(type_model, dtest)
pred_and_test <- cbind.data.frame(predictions, test_label)
confusionMatrix(factor(predictions), factor(test_label))
```

For the purpose of the XGBoost model, we had to refactor our crime code levels from 100-900 to 000-800. Our model predicts the different crime classes with a balanced accuracy of between 0.5 - 0.67, except for crimes in the 700 and 800 range. 
