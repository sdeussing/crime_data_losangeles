...
Let's make a dataframe for these 'worst' clusters.
...
Setup for XGBoost: Premis_Code and District_Num are categorical variables, so they need to be made into dummy variables. 
```{r Setup for XGBoost}
...
tod_model <- xgb.train(data = dtrain, 
                   nrounds = 121, # best_nrounds_tod = best iteration
                   eta = 0.1,
                   objective = "multi:softprob",
                   num_class = 6,
                   eval_metric = "merror")
...
We can make partial dependence plots (pdp) for our most important variable.
```{r}
library(pdp)
...
We see that the Metro System in Los Angeles is the greatest predictor for the hour of crime. This tells us that crimes within this premis are the most identifiable in terms of the hour in which they occur. Such patterns can be helpful for the Los Angeles police department to prepare officers that are stationed on the Metro System about the hours of certain crimes, which can be extracted from this model. 
...
* this is where the issue begins --> the graphs are in the google doc
```{r, eval=FALSE}
...
violent_data$numDayOfWeek_sin <- sapply(violent_data$numDayOfWeek_sin, as.numeric)
violent_data$numDayOfWeek_cos <- sapply(violent_data$numDayOfWeek_cos, as.numeric)
violent_data$TimeOfDay_sin <- sapply(violent_data$TimeOfDay_sin, as.numeric)
violent_data$TimeOfDay_cos <- sapply(violent_data$TimeOfDay_cos, as.numeric)
violent_data$month_sin <- sapply(violent_data$month_sin, as.numeric)
violent_data$month_cos <- sapply(violent_data$month_cos, as.numeric)
```


```{r Violent crime model, eval=FALSE}
...
train_indices <- sample(1:nrow(violent_data), 0.8 * nrow(violent_data))
train_set <- vars[train_indices, ]
test_set <- vars[-train_indices, ]
train_label <- outcome[train_indices]
test_label <- outcome[-train_indices]
...
dtrain <- xgb.DMatrix(data = as.matrix(train_set), label = train_label,
                      weight = weights[as.numeric(train_label) + 1])
dtest <- xgb.DMatrix(data = as.matrix(test_set))
```

Let's tune the number of rounds for this model
```{r tune violence, eval = FALSE}
# Tune number of rounds
...
```{r test violence, eval=FALSE}
# Train the model
violent_model <- xgb.train(data = dtrain, 
                   nrounds = 142, #best_nrounds_violent = best iteration
                   eta = 0.1,
                   objective = "binary:logistic", 
                   eval_metric = "error")
...
We see a balanced accuracy of 0.6869 with this model. Our sensitivity score is 0.66, meaning that our model correctly identifies 66% of non-violent (0) cases as non-violent. Our specificity score is 0.71, meaning that our model correctly identifies 71% of violent (1) cases as violent.  
...
From the premis code plot, we see that premis_codes less than 500 are places in which a violent crime is more likely to occur. We see premis_codes of around 750 as strong predictors for non-violent crimes. Let's look at some of these premis codes.
```{r}
# Violent
crime %>% select(Premis_Code, Premis_Desc) %>% filter(Premis_Code < 500) %>% distinct() %>%sample_n(10)

# Non-violent
crime %>% select(Premis_Code, Premis_Desc) %>% filter(Premis_Code > 720 & Premis_Code < 780) %>% distinct() %>% sample_n(10)
```

From the district number plot, we see that district numbers of ~1300 and ~1800 are places in which a violent crime is more likely to occur. On the other hand, district numbers of ~800 are places in which a non-violent crime is more likely to occur.
...
for (i in seq(0.4, 0.6, by = 0.01)){
  ...
  accuracy <- mean(as.numeric(test_pred) == as.numeric(test_label))
  conf_matrix <- confusionMatrix(factor(test_pred), factor(test_label))
...
We still see a balanced accuracy of 68%. Threshold testing proved that we selected the correct threshold for our model, meaning that we have found the highest balanced accuracy possible.
...
## Additional Discussion & Conclusions
  ...
  We then moved into prediction. These models used the clusters identified as high-crime (4, 10, and 11) for modeling. From our XGBoost models, we learned the most important factors that predict a violent crime and the time of the day that a crime will occur. For both models, we see that premis code is the dominant predictor, with district number also being a significant predictor in our violence model. In terms of practical conclusions, the Los Angeles police department should be prepared to help those in the areas. For those in non-violent areas (as identified by the violence model), officers from these districts should be prepared to help those in more violent districts. 
  - Time of Day Model: Premis codes of ~800 - ~1000.  
  - Violence Model: Premis codes of ~750 have violent crimes. District numbers of ~1300 and ~1800 have violent crimes. 
  
The predicting power of these models allow for the Los Angeles police department to further their knowledge about crimes (and where/when they are most likely to occur) to increase the department's crime prevention abilities. For example, these models can be applied to specific crime instances. Officers can input information (as the test data) about a crime to see the time of day in which it is most likely to occur. This can be useful when looking at 'worst' crimes (i.e. when is homocide/rape/etc. most likely to occur throughout the day). 
