...
by_age
```
The distribution of victim ages is relatively normal and similar between male and female with the most crimes being committed against males and females aged 25 to 50. From the area of each histogram you can see there is anroughly equal number of crimes committed against males and females. 
```{r Severity of Crimes by Area}
library(dplyr)
indicators <- crime
indicators$Arrest <- ifelse(indicators$Status == 'JA' | indicators$Status == 'AA', 1, 0)
indicators$Armed <- ifelse(!is.na(indicators$Weapon_Used_Code), 1, 0)
indicators


summary_by_area <- indicators %>%
  group_by(Area) %>%
  summarise(
