---
title: "Analysis of Crime of Los Angeles 2020-2024"
...
```{r Severity of Crimes by Area, warning=FALSE}
library(dplyr)
library(ggmap)
...
```{r Group by District, warning=FALSE}
library(DescTools)
...
```{r PAM K-Means CLustering, warning=FALSE}
library(cluster)
library(tibble)
...
g_heat_2
```
The clusters of interest that we identified:

Group 10- lots of guns/blade/arrests
Group 11- high vulnerability and severity, but low guns/blades/arrests
...
#preview of df with cluster num
head(sum_by_cluster)
```


```{r Clusters by Color Map, warning=FALSE}
...
```{r Put cluster numbers back, warning=FALSE}
clust_crime <- crime %>%
...
```{r Clean Time/Date, warning=FALSE}
library(lubridate)
...
```
#### Time of Day Crime XGBoost Model
##### Predicting Hour
...
```{r XGBoost on districts, warning=FALSE}
library(xgboost)
library(caret)
set.seed(1234567)
...
```{r XGBoost for hour, warning=FALSE}
...
# Create a partial dependence plot for a feature (e.g., "feature1")
