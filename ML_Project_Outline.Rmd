...
library(readxl)
new <- crime
crime_hierarchy <- read_excel('CIBRS Offense Listing.xlsx', sheet = 2)
crime_hierarchy <- crime_hierarchy %>% select(`BCS CODE`, HIERARCHY, `OFFENSE DESCRIPTION`) %>% rename(Crime_Code = `BCS CODE` ) %>% mutate(Crime_Code = as.numeric(Crime_Code))

joined <- left_join(new, crime_hierarchy, by = 'Crime_Code')


#Ranked our crimes in terms of hierarchy and if part 1 or 2 offense
new <- new %>% select(Crime_Code_Desc, Crime_Code, Part_1_2)%>% distinct(Crime_Code_Desc,Crime_Code, Part_1_2) %>%  arrange(desc(Part_1_2), desc(Crime_Code)) %>%  mutate(Severity = row_number())

#Joins on overall CrimeCode
rank_code <- new %>% select(Severity, Crime_Code)
rank_code
left_join(crime, rank_code, by = "Crime_Code")


#REMOVE CRMCODE 4 ALL NAS
#LEFT JOIN ON CRM1, 2, 3 = SEVERITY 1,2, 3
